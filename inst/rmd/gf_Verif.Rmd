---
title: "Vérification des données d'inventaire par placettes permanentes"
output: html_document
---

```{r Chargement, echo=F}
# ----- Library
librarian::shelf(tidyverse, Pplot)
...
# Vérification de la base de données `r Forets$Nom`
...
names(read_excel(fich, sheet = "Arbres", n_max = 0))
...
for (i in 3:6) {
  if (!is.numeric(AccD[,i])) {cat(paste("Attention : la variable",names(AccD)[i],"n'est pas numérique."))}
}
...
if (dim(BMSLineaires)[1] > 0 ){
  ...
  for (i in 6:7) {
    if (!is.numeric(BMSLineaires[,i])) {
      cat(paste("Attention : la variable",names(BMSLineaires)[i],"n'est pas numérique."))
      }
  }
...
if (dim(BMSLineaires)[1] > 0 ){
  t1 <- melt(BMSLineaires[,c("NumForet","Diam","Stade")], id="NumForet")
  ggplot(t1, aes(x=variable, y=value)) + geom_violin(fill = "grey90") + 
    facet_wrap(~ variable, scales = "free", ncol=2)+ theme_bw()
}
```
...
if (dim(IdArbres)[1] > 0 ){
  ...
  for (i in 6:7) {
    if (!is.numeric(IdArbres[,i])) {
      cat(paste("Attention : la variable",names(IdArbres)[i],"n'est pas numérique."))
      }
  }
  ...
  t1 <- melt(IdArbres[,c("NumForet","Azimut","Dist")], id="NumForet")
  ggplot(t1, aes(x=variable, y=value)) + geom_violin(fill = "grey90") + 
    facet_wrap(~ variable, scales = "free", ncol=2)+ theme_bw()
...
} else {cat("Toutes les essences ont un tarif de cubage.")}
...
} else {cat("Toutes les essences ont une mercuriale de prix.")}
Liste <- setdiff(unique(IdArbres$Essence), unique(EssReg$Essence))
if (length(Liste) > 0) {
  print(paste("Il manque l'affectation à une essence regroupée pour les essences suivantes :", Liste))
} else {cat("Toutes les essences ont été regroupées.")}
```
